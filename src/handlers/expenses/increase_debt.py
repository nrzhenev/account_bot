...
DEBT_ACTORS = ["Два Брата", "Gold pack", "Сыр", "Ростислав", "Гоча упаковки", "Овощи"]
DEBT_ACTIONS = ["Увеличить долг", "Уменьшить долг"]


class DebtStates(StatesGroup):
    CHOOSING_DEBT_ACTION = StateWithData("Взять или Вернуть?", keyboard_with_return_button(["Взять в долг", "Вернуть долг"]))
    CHOOSING_DEBT_ACTOR = StateWithData("Кому/у Кого?", keyboard_with_return_button(DEBT_ACTORS))
    CHOOSING_DEBT_RETURN_METHOD = StateWithData("Вернули картой или наличными?", keyboard_with_return_button(["Картой", "Наличными"]))
    CHOOSING_DEBT_QUANTITY = StateWithData("Введите сумму")


expenses_mh.add_transition(ExpensesInitialStates.WAITING_CHOOSE_ACTION,
                           DebtStates.CHOOSING_DEBT_ACTION,
                           "Вернуть/Взять Долг")
expenses_mh.add_transition(DebtStates.CHOOSING_DEBT_ACTION, DebtStates.CHOOSING_DEBT_ACTOR)
expenses_mh.add_transition(DebtStates.CHOOSING_DEBT_ACTOR, DebtStates.CHOOSING_DEBT_RETURN_METHOD)
expenses_mh.add_transition(DebtStates.CHOOSING_DEBT_RETURN_METHOD, DebtStates.CHOOSING_DEBT_QUANTITY)
